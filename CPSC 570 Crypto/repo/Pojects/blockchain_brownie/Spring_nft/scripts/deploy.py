from brownie import NFT2, Token, accounts ,network , config
...
def deploy_token(): # Deploys an ERC20 token contract (UBToken) using the deployer's account .
    ...
    ubtoken = Token.deploy({'from': account}, publish_source=config['networks'][network.show_active()].get('verify', False))
...
def deploy_nft():
    account = get_account()
    baseuri = check_IPFS()
    # if network.show_active() == 'development' or len(Token) == 0: # Checks if the network is development or if the token has not been deployed yet.
    token = deploy_token()
    # else:
    #     token = Token[-1]
