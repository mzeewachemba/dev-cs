...
class Process:
    ...
    def __str__(self):
        ...
        return (
            f"Process {self.process_no} - Allotted: {self.alloted_resource}, Requested: {self.requested_resource} , "
...
class Reduction:
    ...
    def print_dependency_matrix(self, processes, dependency_matrix):
        ...
        # Print the header
        print("\nDependency Matrix:")
        print("    ", " ".join(f"P{num}" for num in process_numbers)) #print top row

        # Print each row with row headers
        for i, row in enumerate(dependency_matrix): #get index and value
...
def main():
    ...
    # file = "./process_resource_allocation_graph_2.txt"

    red = Reduction()
    processes = red.parse_process_resource_allocation_graph(file)

    #STEP 01 - looking at every pair of processes to see if they depend on each other
    ...
    dependencies = red.transform_to_dependency_list(processes_with_dependencies) #ret tuple of list of processes and their dependencies
    # red.print_dependencies(dependencies)

    # STEP 02 - Detect if there is a cycle in the dependency graph
    cycle = Cycle()  # Create an instance of the Cycle class
    graph = cycle.create_graph(dependencies)
    cycles = cycle.detect_cycles(graph)
    non_redundant_cycles = cycle.process_cycles(cycles)

    #STEP 03 - Determining pre emption needed for each process
    cycle.break_cycle(non_redundant_cycles, processes_with_dependencies)
