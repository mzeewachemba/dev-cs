...
class Job:
    ...
    def __str__(self):
        return (f"Job #{self.job_number} - Arrival: {self.arrival_time:.2f}, "
                ...
                f"Queue: {self.queue_number}, Status: {self.status.value}")
...
class MultiLevelQueueScheduling:
    def __init__(self):
        pass
    def process_jobs(self, jobs, context_switching_time , priority_based_switch):
        pass
def main():
    jobs = []
    ...
    context_switching_time = int(input(f"{YELLOW}Enter Context Switching Time{RESET}"))
