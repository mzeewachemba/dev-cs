...
class MultiLevelQueueScheduling(LinearQueue):
    ...
    def plot_optimization(self, jobs ):
        ...
        plt.annotate(f'{avg_turnaround_times:.2f}', (time, avg_turnaround_times), textcoords="offset points",
                     xytext=(0, -5), ha='left')
        plt.annotate(f'{avg_waiting_times:.2f}', (time, avg_waiting_times), textcoords="offset points", xytext=(0, -5),
                     ha='right')
        plt.annotate(f'{cpu_utilizations:.2f}%', (time, cpu_utilizations), textcoords="offset points", xytext=(0, -5),
                     ha='left')
        plt.annotate(f'{max_turnaround_times:.2f}', (time, max_turnaround_times), textcoords="offset points",
                     xytext=(0, -5), ha='right')
        plt.annotate(f'{max_waiting_times:.2f}', (time, max_waiting_times), textcoords="offset points", xytext=(0, -5),
                     ha='left')
        plt.annotate(f'{cpu_throughputs:.2f}', (time, cpu_throughputs), textcoords="offset points", xytext=(0, -5),
                     ha='right')
