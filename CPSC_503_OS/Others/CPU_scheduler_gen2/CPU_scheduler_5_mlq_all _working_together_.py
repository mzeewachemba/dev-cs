...
class FIFO(LinearQueue):
    ...
    def get_avg_wt(self):
        for job in self.tracked_jobs:
            average_waiting_time = sum((job.exit_time - job.arrival_time) - job.actual_execution_time) / len(self.tracked_jobs)
            return average_waiting_time

    def get_avg_turn_t(self):
        for job in self.tracked_jobs:
            average_turnaround_time = sum(job.exit_time - job.arrival_time) / len(self.tracked_jobs)
    ...
    def plot_aging_threshold(self, max_threshold=10):
        ...
        #
        # for threshold in aging_threshold_values:
        #     self.aging_threshold = threshold
        #     self.process_jobs(jobs, context_switching_time)
        #     avg_waiting_times.append(self.get_avg_waiting_time())
        #     avg_turnaround_times.append(self.get_avg_turnaround_time())
        #
        # plt.figure(figsize=(10, 6))
        # plt.plot(aging_threshold_values, avg_waiting_times, label='Average Waiting Time')
        # plt.plot(aging_threshold_values, avg_turnaround_times, label='Average Turnaround Time')
        # plt.xlabel('Aging Threshold')
        # plt.ylabel('Time (units)')
        # plt.title('Effect of Aging Threshold on Average Waiting Time and Turnaround Time')
        # plt.legend()
        # plt.grid(True)
        # plt.show()
