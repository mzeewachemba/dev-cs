...
def main():
    ...
    criterion = nn.CrossEntropyLoss()  # for multiclass classification, cross entropy loss works better
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    num_total_steps = len(train_loader)
    
    for epoch in range(num_epochs):
        for i, (x, labels) in enumerate(train_loader):
            x = x.reshape(-1, 1, x.size(1)).to(device)  # convert to CPU or GPU tensor
