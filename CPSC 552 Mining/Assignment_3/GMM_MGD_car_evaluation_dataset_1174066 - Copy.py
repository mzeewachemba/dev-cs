...
def main():
    ...
    cluster_assigned = [mode(preds[0:500])[0], mode(preds[500:1000])[0], mode(preds[1000:1500])[0] ,mode(preds[1500:1727])[0]]
    acc = 0
    #checking if our predictions are present in the clusters
    for i in range(n):
        if preds[i] == cluster_assigned[0] and i < 500:
            acc += 1
        if preds[i] == cluster_assigned[1] and 500 <= i < 1000:
            acc += 1
        if preds[i] == cluster_assigned[2] and 1000 <= i < 1500:
            acc += 1
        if preds[i] == cluster_assigned[3] and 1500 <= i < 1727:
            acc += 1
