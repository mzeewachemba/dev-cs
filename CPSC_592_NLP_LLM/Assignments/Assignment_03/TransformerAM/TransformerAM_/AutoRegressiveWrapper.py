...
def top_k(logits, thres=0.9):
    k = int((1 - thres) * logits.shape[-1])  # top 10% logits
    val, ind = torch.topk(logits, k)  # Get top-k values and their indices
    ...
    return probs  # i.e., top 10% locations will have values, rest -inf
...
class AutoRegressiveWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.model(xi)
        logits_reorg = out.view(-1, out.size(-1))  # Reshape for cross-entropy loss
        targets_reorg = xo.reshape(-1)  # Reshape for cross-entropy loss
        loss = F.cross_entropy(logits_reorg, targets_reorg)  # Calculate loss
        return loss
