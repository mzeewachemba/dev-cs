...
def main():
    ...
    nvidia_gpu_model_used = torch.cuda.get_device_name(0)
    print(f"Student ID: 1174066 , MODEL RUN ON: {nvidia_gpu_model_used}\n")
    ...
    #dbugging loader
    # Display a few batches from the training loader
    print("\nTraining Batches:")
    for i, batch in enumerate(train_loader):
        if i >= 1:  # Display only the first 3 batches
            break
        print(f"Batch {i + 1}:\n{batch}")
        
    # Display a few batches from the validation dataset
    print("\nDataset Batches:")
    ...
    # Display a few batches from the validation loader
    print(f"\nValidation Batches:")
    ...
    # print(f"\nMemory: {torch.cuda.memory_summary(device=None, abbreviated=False)}")
